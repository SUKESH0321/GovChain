...
async function createProject(req, res) {
  try {
    ...
    // Blockchain recording runs only AFTER the project was stored in PostgreSQL.
    // The service never throws: a failed transaction is stored in
    // blockchain_events with status 'FAILED' and returned here, so the project
    // itself is never rolled back because of the blockchain.
    const blockchain = await blockchainService.recordProjectCreated({
      projectId: project.id,
      actorUserId: req.user.id,
    });

    return res.status(201).json({ message: 'Project created successfully', project, blockchain });
  } catch (error) {
...
async function updateProject(req, res) {
  try {
    ...
    // The update reached PostgreSQL first; the audit record is written to the
    // blockchain afterwards (updated_at keeps repeated updates distinct).
    const blockchain = await blockchainService.recordProjectUpdated({
      projectId: project.id,
      actorUserId: req.user.id,
      projectUpdatedAt: project.updated_at,
    });

    return res.json({ message: 'Project updated successfully', project, blockchain });
  } catch (error) {
