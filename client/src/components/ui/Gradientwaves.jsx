...
void main() {
  ...
  vec3 body = mix(uWaveColor, uCrestColor, clamp(pos.z * 0.08 + 0.4, 0.0, 1.0));
  
  vec3 col = mix(uHorizonColor, body, smoothstep(0.0, 0.45, screenY));
  col = mix(col, uCrestColor, smoothstep(0.55, 1.0, screenY));
...
const GradientWaves = ({
  ...
  // 'canvas' — track the pointer on the canvas itself (default).
  // 'window' — track the pointer on the window, so the canvas can stay
  //            `pointer-events: none` and still respond to cursor movement.
  ...
  useEffect(() => {
    ...
    // Pointer tracking source: the canvas itself, or the window (used when the
    // canvas is a passive background layer with `pointer-events: none`).
    const trackOnWindow = pointerTarget === 'window';
    const pointerSource = trackOnWindow ? window : canvas;
    ...
    // `pointerleave` is an element event; window tracking simply keeps the last
    // cursor position instead of recentering.
    if (!trackOnWindow) pointerSource.addEventListener('pointerleave', onPointerLeave);

    let raf = 0;
    let isVisible = true;
    let isPageVisible = !document.hidden;
    const motionQuery = window.matchMedia('(prefers-reduced-motion: reduce)');
    let reducedMotion = reducedMotionRef.current && motionQuery.matches;
    ...
    const tryStart = () => {
      // Reduced-motion users get a single static frame instead of animation.
      if (isVisible && isPageVisible && !reducedMotion && raf === 0) {
        raf = requestAnimationFrame(loop);
      }
    };
    const tryStop = () => {
    ...
    // Follow live changes to the OS/browser motion preference.
    const onMotionPreference = () => {
      reducedMotion = reducedMotionRef.current && motionQuery.matches;
      if (reducedMotion) {
        tryStop();
        renderer.render({ scene: mesh });
      } else {
    ...
    return () => {
      ...
      if (!trackOnWindow) pointerSource.removeEventListener('pointerleave', onPointerLeave);
